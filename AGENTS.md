...
- 笔试组卷：paper_type=1 科目一+二、2 科目三+四；题型题量 单选45+多选35+判断40（库存充足，两卷均完整 120 题）；165 分钟；分值 单选1/多选1/判断0.5（满分 100）
...
| `questions_1..4.json` | 按科目分文件（前端按需懒加载）——**主库=近5年真题层(2021-2025) + 无年份练习层** |
| `data_legacy/legacy_questions_1..4.json` | **历史层归档（有年份且≤2020 + 2026 模拟卷，约 1.7 万题）**：git 保留、不进构建、前端不加载；需恢复时把对应题移回主库即可 |

> **年份分层约定**：主库只放 2021-2025 真题与无年份练习（笔试组卷库存充足，已验证）；有明确年份且 ≤2020 的题（老模拟卷/章节练习）一律归 `data_legacy/` 归档，不在主库出现。加新题同理：2021+ 或 无年份 → 主库，否则 → 归档文件。
...
- 笔试两套卷判断题库存均充足（科目一+二 51 道、科目三+四 237 道），组卷完整 120 题 / 100 分
- **主库按年份分层（2026-08-26）**：主库只含 2021-2025 真题 + 无年份练习（约 2023 题含 fixture）；有年份 ≤2020 的历史题与 2026 模拟卷全部归档到 `data_legacy/`（约 1.7 万题，git 保留、不进构建）。若需要历史题可随时从归档移回
