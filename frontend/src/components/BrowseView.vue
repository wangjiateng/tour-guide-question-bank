...
async function load() {
  ...
  try {
    ...
    if (sourceFilter.value != null) params.set("source_id", String(sourceFilter.value));
    ...
    if (yearFilter.value != null) params.set("year", String(yearFilter.value));
...
<style scoped>
.filters { display: flex; gap: 16px; align-items: center; margin-bottom: 12px; }
.error { color: var(--bad); }
.card {
...
.qtext { margin: 0 0 8px; line-height: 1.7; }
.subject-chip {
...
.opts li { display: flex; gap: 8px; align-items: baseline; }
.letter {
  flex: none;
  width: 22px;
  height: 22px;
...
.expl { color: var(--muted); margin: 4px 0; }
...
@media (max-width: 640px) {
  ...
  .pager button { min-height: 44px; padding: 4px 16px; }
}
</style>
